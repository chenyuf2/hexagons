    ...
    for (let j = 0; j < NUM_COL; j++) {
      for (let i = 0; i < NUM_ROW; i++) {
        if (j % 2 === 0) {
          ...
          pos[3 * (i * NUM_COL + j)] = j * (1.5 + (Math.sqrt(3) * GAP) / 2);
          pos[3 * (i * NUM_COL + j) + 1] =
            -Math.sqrt(3) / 2 - GAP / 2 + Math.sqrt(3) * i + GAP * i;
        }
    ...
    for (let i = 0; i < NUM_ROW; i++) {
      for (let j = 0; j < NUM_COL; j++) {
        ...
        const distance = Math.sqrt(
          (defaultPos[0] - x) ** 2 + (defaultPos[1] - y) ** 2
        );
        if (distance < RADIUS) {
          scales[i * NUM_COL + j] =
            1 + 0.2 * Math.cos((distance * (Math.PI / 2)) / RADIUS) ** 12;
        }
      }
    }
    return scales;
  ...
  const getAnimatedPosition = (x, y) => {
    ...
    for (let i = 0; i < NUM_ROW; i++) {
      for (let j = 0; j < NUM_COL; j++) {
        ...
        const scaleValue = (0.6 * (1 - distance / 30)) ** 2;
        positions[3 * (i * NUM_COL + j)] =
          defaultPos[0] + vector.x * scaleValue;
        positions[3 * (i * NUM_COL + j) + 1] =
          defaultPos[1] + vector.y * scaleValue;
      }
    }
    return positions;
  };
